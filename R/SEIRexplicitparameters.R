...
#' @param R recovered
#' @param D dead
#' @param tt_matrix tt_matrix
#' @param mix_mat_set mixing matrix
#' @param tt_beta tt_beta
#' @param beta_set beta_set
#' @param i element index
#' @param N_age population for age that is being analysed
#'
#' @return list
#' @export
SEIRexplicitparameters <- function(S, E1, E2, IMild, ICase1, ICase2, cum_hosp_inc, IOxGetLive1, IOxGetLive2,
                                   ...
                                   IMVNotGetDie1, IMVNotGetDie2, IRec1, IRec2, R, D, tt_matrix, mix_mat_set, tt_beta, beta_set, i, N_age,
                                   lambda, problambda) {
  ...
  function(api) {
    ...
    tt_matrix <- seq(1, length(mix_mat_set))

    m <- approxfun(tt_matrix, mix_mat_set, method = "constant")

    # Interpolation for
    beta <- 0
    print(beta_set)
    print(tt_beta)
    if (!is.null(tt_beta) && !is.null(beta_set)) {

      beta <- approxfun(tt_beta, beta_set, method = "constant") #,
                        #yleft, yright, rule = 1, f = 0, ties = mean, na.rm = TRUE)
    }


    # Probability for FOI associated with age - age group population$age_group[i]
    # prob <- runif(1, 1, round(population$age_group[i]))
    # prob <- prob/round(population$age_group[i])

    # Generating Force of Infection

    temp[] <- api$get_state(human, IMild)  + api$get_state(human, ICase1) + api$get_state(human, ICase2)
    #s_ij[,] <- m[i, j] * prob * temp[j]
    for (j in seq_along(N_age)) {
      s_ij[i,j] <- m[i, j] * temp[j]
    }
...
#' @param ICase2 Second of the compartments for infections that will require hospitalisation
#' @param cum_hosp_inc cumulative hosptal incidences
#' @param tt_matrix time
#' @param mix_mat_set contact matrix
#' @param tt_beta time beta
#' @param beta_set beta_set
#' @param i index
#' @param N_age number of points for a particular age
#' @param lambda FOI
#' @param problambda probability of FOI
#'
#' @export
#'
#' @examples
#' SEIRexplicitparameters2(IMild, ICase1, ICase2, cum_hosp_inc,  tt_matrix, mix_mat_set, tt_beta, beta_set, i, N_age,lambda, problambda)
SEIRexplicitparameters2 <- function(IMild, ICase1, ICase2, cum_hosp_inc,  tt_matrix, mix_mat_set, tt_beta, beta_set, i, N_age,
                                   lambda, problambda) {
  function(api) {
#
#     ##Compute the Force of infection
#     # Interpolation for Mixing Matrix
#     m <- vector(mode = "list", length = N_age)
#     print("length m")
#     print(length(m))
#     print("length mix_mat_set")
#     print(length(mix_mat_set))
#     print("length tt_matrix")
#     print(length(tt_matrix))
#     #
    # m <- approxfun(tt_matrix, mix_mat_set, method = "constant")
    #
    ...
    # print("length temp")
    # print(length(temp))
    # s_ij <- vector(mode = "list", length = N_age)
    #
    #
    # if(!isEmpty(api$get_state(human, IMild)) && !isEmpty(api$get_state(human, ICase1)) && !isEmpty(api$get_state(human, ICase2))){
    #   temp <- api$get_state(human, IMild)  + api$get_state(human, ICase1) + api$get_state(human, ICase2)
    # }
    #
    # print(api$get_state(human, IMild))
    # print(api$get_state(human, ICase1))
    # print(api$get_state(human, ICase2))
    # print("length temp")
    # print(length(temp))
    #
    # print("m")
...
#     #s <- mapply("*", m, temp, SIMPLIFY = FALSE)
#     #for (i in N_age){
#       s_ij <- m * temp
#    # }
# print(s_ij)
...
#     print(lambda)
    ...
    problambda <- 0.1  #<- 1 - exp(-lambda * dt)
