...
#' @noRd
assert_neg <- function(x, zero_allowed = TRUE, message1 = "%s must be less than
                       or equal to zero",
...
# x is positive (with or without zero allowed)
#' @noRd
assert_pos <- function(x, zero_allowed = TRUE, message1 = "%s must be greater
                       than or equal to zero", message2 = "%s must be greater
                       than zero", name = deparse(substitute(x))) {
  assert_numeric(x, name = name)
  if (zero_allowed) {
    if (!all(x >= 0)) {
      stop(sprintf(message1, name), call. = FALSE)
...
#' @noRd
assert_single_pos <- function(x, zero_allowed = TRUE,
                              name = deparse(substitute(x))) {
  assert_length(x, n = 1, name = name)
...
#' @noRd
assert_single_pos_int <- function(x, zero_allowed = TRUE,
                                  name = deparse(substitute(x))) {
  assert_length(x, n = 1, name = name)
...
#' @noRd
assert_single_bounded <- function(x, left = 0, right = 1, inclusive_left = TRUE,
                                  inclusive_right = TRUE,
                                  name = deparse(substitute(x))) {
  assert_length(x, n = 1, name = name)
  assert_bounded(x, left = left, right = right, inclusive_left = inclusive_left,
                 inclusive_right = inclusive_right, name = name)
  return(TRUE)
}
...
assert_custom_class <- function(x, c, message = "%s must inherit from class
                                '%s'", name = deparse(substitute(x))) {
  if (!inherits(x, c)) {
...
assert_limit <- function(x, message = "%s must be a valid plotting limit,
                         i.e. contain two increasing values",
                         name = deparse(substitute(x))) {
...
#' @noRd
assert_bounded <- function(x, left = 0, right = 1, inclusive_left = TRUE,
                  inclusive_right = TRUE, name = deparse(substitute(x))) {
  assert_numeric(x, name = name)
  if (inclusive_left) {
    if (!all(x >= left)) {
...
#' @noRd
assert_same_length <- function(x, y, message =  "%s and %s must be the same
                               length",
                               name_x = deparse(substitute(x)),
...
# is symmetric matrix
#' @noRd
assert_symmetric_matrix <- function(x, message = "%s must be a symmetric
                                    matrix", name = deparse(substitute(x))) {
